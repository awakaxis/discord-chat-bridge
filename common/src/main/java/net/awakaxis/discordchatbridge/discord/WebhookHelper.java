...
public final class WebhookHelper {
    ...
    private static WebhookMessage buildForPlayer(String username, String content) {
        return new WebhookMessageBuilder().setUsername(username).setAvatarUrl(getPlayerHeadshot(username)).setContent(content).build();
    }

    private static WebhookMessage buildForServer(String content) {
        return new WebhookMessageBuilder().setUsername("Server").setAvatarUrl(SERVER_AVATAR).setContent(content).build();
    }
}
